...
The ACP spec (agentclientprotocol.com) does NOT define a native "diff" content type. The `type: "diff"` content block is a custom extension built into murder-ide. The protocol defines: `text`, `image`, `audio`, `resource`, `resource_link`. This is fine — it's our own convention.

---
title: Infodump on huge TODO regarding crow-ui
date: "2026-05-07"
---
...
19. Expose backend service controlling sessions — just like we need a backend modification to store data for a queue of messages to send to an agent after receiving end_turn from the agent for non-blocking communication, we also need a similar endpoint for session/cancel. This will allow programatic control over ACP agent servers from the backend which we can wire into an MCP server to hand to a special agent that orchestrates the other agents and can cause clicks anywhere in the interface. drag objects to other locations. interact with the frontend in any way almost like a little playwright agent but it's all done in the backend because we did this right and every click, every button press in the frontend corresponds to some API call to the backend that sets the state on the view. that's where the chat and IDE objectives align and the whole point of me creating my own client. because in ACP CLIENTS are the orchestration mechanism. I was working on trying to turn an ACP agent into an agent-client that can orchestrate other agents and I was like dude build a fucking ACP client and put it behind an API so the MCP you hand to orchestrator can session/prompt and session/cancel. I guess we'd need some form of storing the session/updates for viewing, but I'm not really planning to build this for random ACP agent servers. This is for crow-cli. This is crow-cli's wrapper. But we want it to look freaking SIIICK because what's the point of saying "fuck TUIs give me an IDE" if we end up building a GUI that looks like a freaking TUI? I mean I think I know why we ended up getting the retro-ish IDE look I hated that looked sort of like a TUI with v1 and it's because I was handing it screenshots of zed. there's a reason I'm trying to run from zed even though I'm basically like "okay copy everything zed does" because I've been using this instead of vscode for over a year now so vscode feels sort of awkward to me and the web app is sort of awkward too. 
20. We do NOT want chat bubbles: okay I do NOT want message bubbles. I fucking hate mesage bubbles.  needing margins for the agent output != message bubbles. they're so shitty. I hate them. we just need margins aka better streamdown rendering. NOT message bubbles. the only message that needs to be in a bubble is the user's message can be in a container that's distinct but otherwise let them be don't try to bubble my ACP client chat bro. I like zed's "everything is markdown preview" approach, it just needs polish
...
- put in an electron app and remove the menu box at top of electron app. bring into our app, otherwise treat electron node backend as dumb server to run our server that serves the embedded static compiled frontend
...
so yeah this is a huge infodump


what do you feel like would give us the biggest like "wow this looks mega professional!" lift, visually?


I feel like the black background of the chat should be something much more mellow and I'm not pitching a fit about shades of purple right now because I strategically decided to pick my battles around tailwind, shadcn/ui, and radix rather than replicating shades of purple because replicating shades of purple falls out the backend of the configurability fight.

- we need the jsonc configuration from ~/.crow/murder.json or probably be better to just call it ~/.crow/settings.json


in fact I think murder is going to be the development name of the orchestration tool and this is going to be crow-ui. or something like that. crow-cli runs the backend of crow-ui. works.


Dude this is actually pretty killer not going to lie.

modified with crow-cli through crow-ui!

update monaco diffs to only show active parts of edit in the diff

---

# Monaco Diff Analysis — Active-Only Diff Display

## Current State

The diff rendering lives in `frontend/src/components/FileViews.tsx` → `FileEditView` component. Three places use it:
- `ChatSessionBody.tsx` (line 703)
- `ChatSessionPane.tsx` (line 888)
- `ChatPane.tsx` (similar pattern)

**Data flow:**
1. ACP tool call comes in with `content` blocks (type: "diff" or text)
2. Frontend extracts file path from tool title or diff content block
3. Frontend fetches `beforeContent` from its own file cache (or reads via `read_file` RPC)
4. `afterContent` is the tool's output content
5. Both are passed to `<FileEditView beforeContent={beforeContent} afterContent={fileContent} path={filePath} />`
6. `FileEditView` creates two Monaco models and a `createDiffEditor` with them

**Current `FileEditView` options:**
```ts
const diffEditor = monaco.editor.createDiffEditor(containerRef.current, {
  ...
  hideUnchangedRegions: { enabled: false },  // ← THIS IS THE PROBLEM
});
```

The editor shows the **entire file** side-by-side with highlighted changes. For large files with small edits, this is overwhelming.

## Monaco API Research Findings

### `hideUnchangedRegions` option (supported in monaco-editor 0.55.1 ✅)

The project uses `monaco-editor ^0.55.1`, which fully supports `hideUnchangedRegions`. This option has four sub-properties:

| Property | Type | Default | Description |
|---|---|---|---|
| `enabled` | boolean | false | Toggle unchanged region collapsing |
| `contextLineCount` | number | 3 | Lines of context shown above/below each change hunk |
| `minimumLineCount` | number | 4 | Minimum unchanged region size to be eligible for collapsing |
| `revealLineCount` | number | 10 | Number of lines shown when user clicks to expand a collapsed region |

This was introduced as experimental (`diffEditor.experimental.collapseUnchangedRegions`) in VSCode 1.81 and shipped as stable `hideUnchangedRegions` in Monaco 0.44+.

### Known Issues (from GitHub #4196, #548)

- **Did NOT work in monaco-editor < 0.44.0** — the option was accepted but silently ignored
- **Works fine with direct Monaco import** (not through `@monaco-editor/react` wrapper in older versions)
- Since we use monaco-editor directly (`import * as monaco from "monaco-editor"`), we're fine at 0.55.1

### `renderSideBySide` vs Inline mode

Monaco also supports `renderSideBySide: false` for inline diff view (single editor, original lines shown as deleted, modified lines as added). This is closer to VSCode's newer inline diff experience. For chat context, inline might be more space-efficient.

### ACP Protocol Note

The ACP spec (agentclientprotocol.com) does NOT define a native "diff" content type. The `type: "diff"` content block is a custom extension built into crow-ui-ide. The protocol defines: `text`, `image`, `audio`, `resource`, `resource_link`. This is fine — it's our own convention.

## Planned Changes

### Change 1: Enable `hideUnchangedRegions` in FileEditView

**File:** `frontend/src/components/FileViews.tsx`
...
hideUnchangedRegions: {
  enabled: true,
  contextLineCount: 3,      // Show 3 lines of context around each change
  minimumLineCount: 2,      // Collapse unchanged regions of 2+ lines
  revealLineCount: 5,       // Show 5 lines when expanding
},
```

### Change 2: Add toggle button to expand/collapse

Add a small button in the diff header that toggles `hideUnchangedRegions.enabled` at runtime via `diffEditor.updateOptions()`. This lets users see the full file when needed.

```ts
// Toggle via ref:
diffEditorRef.current?.updateOptions({
  hideUnchangedRegions: { enabled: !currentlyEnabled }
});
```

### Change 3: Consider inline mode for chat context

For the chat panel (narrow space), `renderSideBySide: false` may be preferable. Could make this configurable or auto-switch based on container width using `renderSideBySideInlineBreakpoint`.

### Change 4: Handle streaming/progressive diffs

Currently the diff view only appears when both `beforeContent` AND `fileContent` are available. For streaming agent responses, we should consider:
- Showing a partial diff as content streams in
- Using `stream-monaco` library (github.com/Simon-He95/stream-monaco) which has built-in support for folding unchanged regions during streaming

### Change 5: Height calculation fix

Current height calculation `Math.max(beforeContent.split("\n").length, afterContent.split("\n").length) * 18 + 16` doesn't account for collapsed regions. With `hideUnchangedRegions: true`, the visible height will be much smaller, causing excess empty space. Should switch to `automaticLayout` with a `maxHeight` CSS constraint instead of fixed height.

...
pub fn handle_workspace_open(state: &AppState, params: &Value) -> Result<Value, String> {
    ...
    let _ = crow_ui_db::recent::add_recent_workspace(&state.db.lock(), path);
...
pub fn handle_terminal_spawn(state: &AppState, params: &Value) -> Result<Value, String> {
    ...
    config.size = crow_ui_terminal::TerminalSize { rows, cols };
...
pub fn handle_terminal_write(state: &AppState, params: &Value) -> Result<Value, String> {
    ...
    tm.write(crow_ui_terminal::TerminalId(id), data)
        .map_err(|e| e.to_string())?;
    Ok(json!({ "success": true }))
}
...
pub fn handle_terminal_resize(state: &AppState, params: &Value) -> Result<Value, String> {
    ...
    let tm = state.terminals.lock();
    tm.resize(
        crow_ui_terminal::TerminalId(id),
        crow_ui_terminal::TerminalSize { rows, cols },
    )
    .map_err(|e| e.to_string())?;
    Ok(json!({ "success": true }))
}
...
pub fn handle_terminal_info(state: &AppState, params: &Value) -> Result<Value, String> {
    ...
    let tm = state.terminals.lock();
    let info = tm
        .info(crow_ui_terminal::TerminalId(id))
        .map_err(|e| e.to_string())?;
    Ok(json!({
        "id": info.handle.0,
        "shell": info.shell,
...
pub async fn handle_get_file_change(state: &AppState, params: &Value) -> Result<Value, String> {
    ...
    if let Some(c) = change {
        Ok(json!({
            ...
            "kind": match c.kind {
                crow_ui_workspace::FileEventKind::Created => "created",
                crow_ui_workspace::FileEventKind::Modified => "modified",
                crow_ui_workspace::FileEventKind::Deleted => "deleted",
                crow_ui_workspace::FileEventKind::Renamed => "renamed",
            },
        }))
    } else {
...
pub fn handle_get_recent_workspaces(state: &AppState, params: &Value) -> Result<Value, String> {
    ...
    let workspaces = crow_ui_db::recent::recent_workspaces(&state.db.lock(), limit)
...
pub fn handle_add_recent_workspace(state: &AppState, params: &Value) -> Result<Value, String> {
    let path = params["path"].as_str().ok_or("missing 'path'")?;
    crow_ui_db::recent::add_recent_workspace(&state.db.lock(), path)
...
pub fn handle_save_workspace_layout(state: &AppState, params: &Value) -> Result<Value, String> {
    ...
    crow_ui_db::layout::save_workspace_layout(&state.db.lock(), &workspace, layout_json)
...
pub fn handle_save_explorer_state(state: &AppState, params: &Value) -> Result<Value, String> {
    ...
    crow_ui_db::layout::save_explorer_state(&state.db.lock(), &workspace, expanded_dirs, active_file)
...
pub fn handle_save_tile_state(state: &AppState, params: &Value) -> Result<Value, String> {
    ...
    crow_ui_db::layout::save_tile_state(&state.db.lock(), &workspace, tile_id, tile_type, state_json, is_minimized)
...
pub fn handle_delete_tile_state(state: &AppState, params: &Value) -> Result<Value, String> {
    let workspace = resolve_workspace(state, params)?;
    let tile_id = params["tileId"].as_str().ok_or("missing 'tileId'")?;
    crow_ui_db::layout::delete_tile_state(&state.db.lock(), &workspace, tile_id)
